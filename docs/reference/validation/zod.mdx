...
`validator` と `handler` を key に持つ**オブジェクトが `defineController` に** [`ServerMethodHandler`](/docs/reference/controller#object-servermethodhandler) として**渡されると**、Frourio は [`ServerHandler`](/docs/reference/controller#function-serverhandler) の引数 `Requestparams` を検証します。

`valiator` は `RequestParams` と同じ key を持ち、**zod オブジェクトスキーマ** (`z.object()`) を value に持つオブジェクトです。
...
### `defineValidators` 関数 {#function-definevalidators}
